...
import { useCreateDish } from "@/pages/Admin/hooks/DishesMutations";
import { useAddDishStore } from "@/pages/Admin/store/AddDishModal.store";
...
export const CreateDishModal: FC<Props> = ({ token }) => {
  const { mutateAsync: CreateDish } = useCreateDish();
  const formRef = useRef<HTMLFormElement>(null);
  const { close, isOpen, categoryId } = useAddDishStore();
    ...
    <Modal isOpen={isOpen} closeModal={closeHandle}>
      <form className={cl.form} onSubmit={submitHandle} ref={formRef}>
        ...
        {categoryId ? (
          ...
          <p>Ошибка: неизвестная категория</p>
        )}
      </form>
    </Modal>
  );
